...
class ASFFV5(nn.Module):
    def __init__(self, level, ch, multiplier=1, rfb=False, vis=False, act_cfg=True):
        ...
        if level == 0:  # P5
            ...
            self.compress_level_0 = Conv(int(ch[3] * multiplier), self.inter_dim, 1, 1)
            self.compress_level_1 = Conv(int(ch[2] * multiplier), self.inter_dim, 1, 1)
            self.stride_level_3 = Conv(int(ch[0] * multiplier), self.inter_dim, 3, 2)
            self.expand = Conv(self.inter_dim, int(ch[1] * multiplier), 3, 1)
        elif level == 3:  # P2
            self.compress_level_0 = Conv(int(ch[3] * multiplier), self.inter_dim, 1, 1)
            self.compress_level_1 = Conv(int(ch[2] * multiplier), self.inter_dim, 1, 1)
            self.compress_level_2 = Conv(int(ch[1] * multiplier), self.inter_dim, 1, 1)
            self.expand = Conv(self.inter_dim, int(ch[0] * multiplier), 3, 1)

        # when adding rfb, we use half number of channels to save memory
        compress_c = 8 if rfb else 16

        self.weight_level_0 = DynamicConv(
            self.inter_dim, compress_c, kernel_size=1, stride=1, num_experts=4)
        self.weight_level_1 = DynamicConv(
            self.inter_dim, compress_c, kernel_size=1, stride=1, num_experts=4)
        self.weight_level_2 = DynamicConv(
            self.inter_dim, compress_c, kernel_size=1, stride=1, num_experts=4)
        self.weight_level_3 = DynamicConv(
            self.inter_dim, compress_c, kernel_size=1, stride=1, num_experts=4)
    ...
    def forward(self, x):  # l,m,s
        ...
        if self.level == 0:  # P5
            ...
            x_level_1_compressed = self.compress_level_1(x_level_1)  
            level_1_resized = F.interpolate(x_level_1_compressed, scale_factor=4, mode='nearest')  
        ...
        # print('level: {}, l1_resized: {}, l2_resized: {}, l3_resized: {}'.format(self.level,
        #      level_1_resized.shape, level_2_resized.shape, level_3_resized.shape))
        # exit()
        ...
        # print(f"level_0_resized: {level_0_resized.shape}")
        # print(f"level_1_resized: {level_1_resized.shape}")
        # print(f"level_2_resized: {level_2_resized.shape}")
        # print(f"level_3_resized: {level_3_resized.shape}")
        ...
        levels_weight_v = torch.cat(
            (level_0_weight_v, level_1_weight_v, 
             level_2_weight_v, level_3_weight_v), 1)
...
if __name__ == "__main__":
    ...
    image1 = (1, 32, 64, 64)  # P2
    image2 = (1, 64, 32, 32)  # P3
    image3 = (1, 128, 16, 16)  # P4
    image4 = (1, 256, 8, 8)  # P5
    ...
    model = Detect_ASFF(nc=80, ch=channel)
